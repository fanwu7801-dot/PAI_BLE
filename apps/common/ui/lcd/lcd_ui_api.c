...
int page_scale_move_abort(int cur_win, int mode);
int ui_page_move_stop(int curr_win, int mode);
int page_scale_move_status();
struct element *ui_core_get_root();
int ui_relate_init();
int ui_relate_uninit();
int ui_page_num();
int imb_task_head_status(int index);
...
enum {
    ...
    UI_MSG_PAGE_SCALE_EFFECT,
    UI_MSG_PAGE_SCALE_ABORT,
    UI_MSG_GIF_PLAY,
};
...
__retry:
#if IMB_DEMO_ENABLE
    err =  os_taskq_post_type("imb", msg[0], len - 1, &msg[1]);
#else
...
int page_scale_move_abort(int curr_win, int mode)
{
    OS_SEM sem;
    int msg[8];
    u8 need_pend = 0;

    while (page_scale_move_status()) {
        if (!(cpu_in_irq() || cpu_irq_disabled())) {
            if (strcmp(os_current_task(), UI_TASK_NAME)) {
                need_pend = 1;
            }
        ...
        msg[0] = UI_MSG_PAGE_SCALE_ABORT;
        msg[1] = curr_win;
        msg[2] = mode;
        msg[3] = 0;
        msg[4] = 0;

        printf("abort page_move\n");
...
int ui_page_scale_effect_post(int curr_win, int next_win, int dir)
{
    OS_SEM sem;
    int msg[8];
    u8 need_pend = 0;
    ...
    msg[0] = UI_MSG_PAGE_SCALE_EFFECT;
    msg[1] = curr_win;
    msg[2] = next_win;
    msg[3] = dir;
    msg[4] = 0;
...
int ui_gif_msg_type_get()
{
    ...
    /* printf("%s:%x",__func__,msg_type); */
    return msg_type;
}


int ui_gif_play_post(struct imb_task *task, u32 msec, u32 delay, u32 elm_id, u32 root_index)
{
    u32 rets;
    __asm__ volatile("%0 = rets":"=r"(rets));
    /* printf("__func__ %s %x\n", __func__, rets); */
    ...
    struct gif_info *gif_info = &task->priv_buf[0];
    u16 timer_id = gif_info->gif_timer;
    ...
    msg[5] = root_index;
    msg[6] = 0;
    /* printf("%s msg[0]:%x",__func__,msg[0]); */
...
void gif_play(int root_index, struct imb_task *task, u32 elm_id);
...
u16 file_crc(char *drive, char *file)
{
    u16 page_size = BLOCK_SIZE;
    int currlen;
    u16 crc_interval = 0;
    ...
    int offset = 0;
    u8 *page_buffer = (u8 *)malloc(page_size);
    while (remain_filelen) {
        currlen = (remain_filelen > page_size) ? page_size : remain_filelen;
        fread(fp, page_buffer, currlen);
        crc_interval = CRC16_with_initval(page_buffer, currlen, crc_interval);
        remain_filelen -= currlen;
        offset += currlen;
    }
    fclose(fp);
    free(page_buffer);
...
int file_exist(char *dst_drive, char *file)
{
    char dst_path[64];

    strcpy(dst_path, dst_drive);
    strcat(dst_path, file);

    FILE *fdst = fopen(dst_path, "r");
    if (!fdst) {
        printf("open %s(r) fail!\n", dst_path);
        return -EEXIST;
    }
    fclose(fdst);
    return 0;
...
int file_check(char *src_drive, char *dst_drive, char *file)
{
    char src_path[64];
    char dst_path[64];

    strcpy(src_path, src_drive);
    ...
    u8 *page_buffer = (u8 *)malloc(page_size);
    u8 *page_buffer_check = (u8 *)malloc(page_size);
    int retry = 3;
    int check_pass;
    u16 crc_interval = 0;
    do {
        int filelen = flen(fsrc);
        int remain_filelen = filelen;
        int currlen;
        int offset = 0;
        check_pass = true;
        while (remain_filelen) {
            currlen = (remain_filelen > page_size) ? page_size : remain_filelen;
            fread(fsrc, page_buffer, currlen);
            fread(fdst, page_buffer_check, currlen);

            crc_interval = CRC16_with_initval(page_buffer_check, currlen, crc_interval);

            if (memcmp(page_buffer, page_buffer_check, currlen)) {
                printf("data check fail!\n");
                printf("src data : \n");
                put_buf(page_buffer, currlen);

                printf("dst data : \n");
                put_buf(page_buffer_check, currlen);
                check_pass = false;
                break;
            }
            /* ASSERT(!memcmp(page_buffer, page_buffer_check, currlen)); */
            /* printf("check offset %d pass\n", offset); */
            remain_filelen -= currlen;
            offset += currlen;
        }
        if (check_pass) {
    ...
    free(page_buffer);
    free(page_buffer_check);

    printf("%s crc_interval : 0x%04x\n", dst_path, crc_interval);
...
int file_copy(char *src_drive, char *dst_drive, char *file)
{
    printf("src_drive : %s, dst_drive : %s, file : %s\n", src_drive, dst_drive, file);
    ...
    while (remain_filelen) {
        currlen = (remain_filelen > page_size) ? page_size : remain_filelen;
        fread(fsrc, page_buffer, currlen);
        fwrite(fdst, page_buffer, currlen);
        remain_filelen -= currlen;
        offset += currlen;
    ...
    int ret = file_check(src_drive, dst_drive, file);
    if (ret) {
        return -1;
    }
...
{ \
	void wdt_close(void); \
	wdt_close(); \
	printf("%s, %d. in stop point\n", __func__, __LINE__); \
	while(1); \
}

int nandflash_fat_test()
{
#if (defined TCFG_NANDFLASH_FAT_ENABLE && TCFG_NANDFLASH_FAT_ENABLE)

#define TEST_ROOT	"storage/"TCFG_NANDFLASH_FAT_ROOT"/C/"
#define TEST_PATH	TEST_ROOT"001.txt"

    FILE *testfile;
    printf("nandflash fat test\n");

    /* int ret = f_format("fat_nand", "fat", 2048); */

    /* testfile = fopen(TEST_PATH, "r"); */
    /* if (testfile) { */
    /*     printf("OPEN file %s succ\n", TEST_PATH); */

    /*     u8 t_buf[256] = {0}; */
    /*     fread(testfile, t_buf, 256); */
    /*     put_buf(t_buf, 256); */
    ...
    /* 测试文本 */
    char test_buf[] = "hello,world! wangjianlong@zh-jieli.com\n";

    //创建文件测试
    testfile = fopen(TEST_PATH, "w+");
    if (testfile == NULL) {
        printf("create %s fail!\n", TEST_PATH);
        //创建文件失败
        return -1;
    } else {
        fwrite(testfile, test_buf, sizeof(test_buf));
        fclose(testfile);
    ...
    if (testfile == NULL) {
        ...
        int filelen = flen(testfile);
        printf("filelen = %d\n", filelen);
        u8 *tmpbuf = (u8 *)malloc(filelen);
        fread(testfile, tmpbuf, filelen);
        printf("read file data:\n");
        put_buf(tmpbuf, filelen);
        printf("txt:");
        printf(tmpbuf);

        if (strcmp(test_buf, tmpbuf)) {
...
int sdcard_fat_test()
{
    ...
    printf("sdcard fat test\n");

    //测试文本
    char test_buf[] = "hello,world! wangjianlong@zh-jieli.com\n";

    //创建文件测试
    testfile = fopen("storage/sd0/C/001.txt", "w+");
    if (testfile == NULL) {
        printf("create storage/sd0/C/001.txt fail!\n");
        //创建文件失败
        return -1;
    } else {
        fwrite(testfile, test_buf, sizeof(test_buf));
        fclose(testfile);
        printf("create storage/sd0/C/001.txt succ!\n");
    }

    //读文件测试
    testfile = fopen("storage/sd0/C/001.txt", "r");
    if (testfile == NULL) {
        printf("open storage/sd0/C/001.txt fail!\n");
        //读文件失败
        return -1;
    } else {
        int filelen = flen(testfile);
    ...
    //删除文件
    fdelete_by_name("storage/sd0/C/001.txt");
...
struct file_attrs {
    struct file_attr file[64];
    u16 num;
    u16 crc;
};

struct file_attrs resfiles;

void test_nandflash_file()
{
#undef NANDFLASH_ROOT
#define NANDFLASH_ROOT	"storage/virfat_flash/C/"

    FILE *sty_fp = fopen(NANDFLASH_ROOT"JL/JL.sty", "r");
    if (!sty_fp) {
        printf("open %s fail\n", NANDFLASH_ROOT"JL/JL.sty");
        return;
    }
    printf("open %s succ\n", NANDFLASH_ROOT"JL/JL.sty");

    int datalen = 20480;
    u8 *tmp_buf = (u8 *)malloc(datalen);
    fread(sty_fp, tmp_buf, datalen);
    printf("sty file data : \n");
    put_buf(tmp_buf, 32);
    fclose(sty_fp);
    free(tmp_buf);
}
...
int find_file_index(char *name)
{
    ...
    for (i = 0; i < resfiles.num; i++) {
        printf("match %s <> %s\n", resfiles.file[i].name, name);
        if (!strcmp(resfiles.file[i].name, name)) {
            return i;
        }
    }
    return -1;
...
void save_file_info(int filenum)
{
    int i;
    u16 crc;

    resfiles.num = filenum;
    crc = CRC16((u8 *)&resfiles, sizeof(struct file_attrs) - 2);
    if ((resfiles.crc != crc) || 0) {
        resfiles.crc = crc;
        int ret = syscfg_write(VM_UI_FILE_LIST, &resfiles, sizeof(resfiles));
    }
}

/* #define SRC_DRIVE "storage/virfat_flash/C/"     //norflash */
#define SRC_DRIVE "storage/sd0/C/"              //sdcard
#define DST_DRIVE "storage/nandflash_ftl/C/"    //nandflash
void files_copy()
{
    int i;
    int ret;
    ...
    ret = syscfg_read(VM_UI_FILE_LIST, &resfiles, sizeof(resfiles));
    if ((ret < 0) || (resfiles.crc != (CRC16((u8 *)&resfiles, sizeof(struct file_attrs) - 2)))) {
        printf("read_filelist info err");
        for (i = 0; i < sizeof(filelist) / sizeof(filelist[0]); i++) {
            strcpy(resfiles.file[i].name, filelist[i]);
            resfiles.file[i].crc = 0;
        }
        // ret = syscfg_write(VM_UI_FILE_LIST, &resfiles, sizeof(resfiles));
        save_file_info(sizeof(filelist) / sizeof(filelist[0]));
    } else {
        printf("vm filelist :\n");
        for (i = 0; i < resfiles.num; i++) {
            printf("%s, %04x\n", resfiles.file[i].name, resfiles.file[i].crc);
        }
    }

    for (i = 0; i < sizeof(filelist) / sizeof(filelist[0]); i++) {
        int index = find_file_index(filelist[i]);
        ASSERT(index != -1);

        crc = file_crc(SRC_DRIVE, filelist[i]);

        strcpy(resfiles.file[index].name, filelist[i]);
        if ((resfiles.file[index].crc != crc) || (file_exist(DST_DRIVE, filelist[i]))) { //crc有变化，有可能crc被破坏掉了
            resfiles.file[index].crc = crc;

            if (file_check(SRC_DRIVE, DST_DRIVE, filelist[i]) != 0) { //数据再对比一遍，确实不存在或者不对再拷贝
                ret = file_copy(SRC_DRIVE, DST_DRIVE, filelist[i]);

                if (!ret) {
                    printf("file copy %s succ!\n", filelist[i]);
                    save_file_info(sizeof(filelist) / sizeof(filelist[0]));
                } else {
                    printf("file copy %s fail!\n", filelist[i]);
                }
                ASSERT(!ret, ", file copy fail!");
            }
        }
    }
...
#include "ui_vm/ui_vm.h"
    ...
    while (1) {
        ...
        /* printf("msg_type:%x msg[0]:%x",msg_type,msg[0]); */
        switch (msg_type) { //action
        case UI_MSG_EXIT:
        ...
        case UI_MSG_AUTO_SWPAGE_EFFECT:
            ...
            if ((ui_page_num() > 1) || (ui_page_num() < 1)) {
                printf("exception: ui_page_num = %d\n", ui_page_num());
                if (msg[6]) {
        ...
        case UI_MSG_PAGE_SCALE_EFFECT: {
            int curr_win = msg[1];
            int next_win = msg[2];
            int dir = msg[3];
            if (curr_win == CURR_WINDOW_MAIN) {
                curr_win = ui_get_current_window_id();
            }
#ifdef CONFIG_WATCH_CASE_ENABLE
            if (((ui_id2type(curr_win) == CTRL_TYPE_WINDOW))
                && ((ui_id2type(next_win) == CTRL_TYPE_WINDOW))) {
...
#endif
            ui_page_scale(curr_win, next_win, dir);
            if (msg[4]) {
                os_sem_post((OS_SEM *)msg[4]);
            }
        }
        ...
        case UI_MSG_PAGE_SCALE_ABORT: {
            ...
            ui_page_move_stop(curr_win, mode);
            if (msg[4]) {
                os_sem_post((OS_SEM *)msg[4]);
            }
        }
        ...
        case UI_MSG_GIF_PLAY:
            struct imb_task *task = (struct imb_task *)msg[1];
            u32 msec = msg[2];
            u32 gif_delay = msg[3];
            u32 elm_id = msg[4];
            u32 root_index = msg[5];
            u32 delay = jiffies_msec() - msec;
            int pagemove = ui_core_pagemove_flag();

            if (!pagemove && (delay < gif_delay)) {
                gif_play(root_index, task, elm_id);
            } else {
...
int lcd_ui_init(void *arg)
{
...
#if 0//TCFG_NANDFLASH_DEV_ENABLE
    int ret = nandflash_fat_test();
    if (ret == 0) {
        printf("nandflash test succ!\n");
    } else {
        printf("nandflash test fail!\n");
    ...
    int ret1 = sdcard_fat_test();
    if (ret1 == 0) {
        printf("sdcard test succ!\n");
    } else {
        printf("sdcard test fail!\n");
    }
#else
    int ret1 = 0;
#endif

#if 0//TCFG_NANDFLASH_DEV_ENABLE && TCFG_SD0_ENABLE
    if (!ret && !ret1 && !strcmp(RES_PATH, "storage/nandflash_ftl/C/")) {
        files_copy();
        void test_nandflash_file();
        test_nandflash_file();
    }
