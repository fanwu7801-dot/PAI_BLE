...
int dev_manager_add(char *logo)
{
	...
	char rec_dev_logo[32] = {0};
	snprintf(rec_dev_logo,sizeof(rec_dev_logo), "%s%s", logo, "_rec");
...
static void dev_manager_task(void *p)
{
...
#if (defined TCFG_NANDFLASH_DEV_ENABLE && TCFG_NANDFLASH_DEV_ENABLE && !TCFG_VIRFAT_FLASH_ENABLE)
    /* void wdt_close(void); */
    /* wdt_close(); */
    /* int ret = f_format("nandflash_ftl", "fat", 2048); */
    /* ASSERT(!ret); */
	...
	dev_manager_add(TCFG_NANDFLASH_FAT_ROOT);
	/* dev_manager_set_valid_by_logo(TCFG_NANDFLASH_FAT_ROOT, 0);///将设备设置为无效设备 */
#endif
