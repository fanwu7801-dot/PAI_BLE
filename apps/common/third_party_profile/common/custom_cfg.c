...
#define SDFILE_EXT_RESERVED_ROOT_PATH	SDFILE_MOUNT_PATH"/EXT_RESERVED/"
#endif
#define EXIF_CFG_FALLBACK_PATH	SDFILE_EXT_RESERVED_ROOT_PATH"exif"
extern int broadcast_upgrade_exif_check(u32 *exif_addr, u32 *exif_len) __attribute__((weak));

static FILE *open_exif_cfg_file(const char **used_path)
{
    FILE *fp = fopen(EXIF_CFG_PATH, "r");
    if (fp) {
        if (used_path) {
            *used_path = EXIF_CFG_PATH;
        }
        return fp;
    }

    fp = fopen(EXIF_CFG_FALLBACK_PATH, "r");
    if (fp) {
        if (used_path) {
            *used_path = EXIF_CFG_FALLBACK_PATH;
        }
    }
    return fp;
}
...
{
    const char *path = EXIF_CFG_PATH;
    FILE *fp = open_exif_cfg_file(&path);
    if (fp) {
        cfg_printf("open %s succ", path);
        ...
        *len = attr.fsize;
        ...
        cfg_printf("open %s fail", EXIF_CFG_FALLBACK_PATH);
        *addr = 0;
        *len = 0;
        if (broadcast_upgrade_exif_check) {
            if (!broadcast_upgrade_exif_check(addr, len)) {
                cfg_printf("fallback exif addr:%x len:%x\n", *addr, *len);
            } else {
                cfg_printf("fallback exif check fail\n");
            }
        }
    }
...
u32 ex_cfg_get_start_addr(void)
{
    ...
    if (fp) {
        ...
        if (broadcast_upgrade_exif_check) {
            u32 exif_len = 0;
            if (!broadcast_upgrade_exif_check(&start_addr, &exif_len)) {
                cfg_printf("fallback exif start addr:%x len:%x\n", start_addr, exif_len);
            }
        }
    }
