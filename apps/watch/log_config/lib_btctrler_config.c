...
#else
#if SMBOX_MULTI_BLE_EN || SMBOX_BLE_CLIENT_EN
...
#endif
#if OTA_RX_EN
const uint64_t config_btctler_le_features = LE_ENCRYPTION | LE_DATA_PACKET_LENGTH_EXTENSION | LE_2M_PHY | LL_FEAT_LE_EXT_ADV | LE_CORE_V50_FEATURES;
#else
const uint64_t config_btctler_le_features = LE_ENCRYPTION | LE_DATA_PACKET_LENGTH_EXTENSION | LE_2M_PHY | LL_FEAT_LE_EXT_ADV;
#endif
#endif
#else
...
const int config_btctler_le_rx_nums = 15;
#else
const int config_btctler_le_rx_nums = 32;
#endif
const int config_btctler_le_acl_packet_length = 251;
const int config_btctler_le_acl_total_nums = 32;//watch 增加了主机行为,将acl从原来的5设置为25
#else
const int config_btctler_le_afh_en = 0;//watch 添加了主机行为
// LE RAM Control
const int config_btctler_le_rx_nums = 32;
const int config_btctler_le_acl_packet_length = 251;
const int config_btctler_le_acl_total_nums = 16;
#endif
#endif

#define BLE_STATIC_RAM				//蓝牙使用静态ram
#ifdef BLE_STATIC_RAM
// 更改以下常量时需要同步更改静态buf大小（buf不够的情况下蓝牙初始化会报错：ble need buf size:xxx）
// config_btctler_le_hw_nums,config_btctler_le_rx_nums,config_btctler_le_acl_total_nums,config_btctler_le_acl_packet_length
#if OTA_RX_EN
#define MAX_STATIC_RAM_RTX_BUF		(21340 + 4096)
#else
#define MAX_STATIC_RAM_RTX_BUF		(21340 + 16*1024)
#endif
int bb_static_ram[(MAX_STATIC_RAM_RTX_BUF + 3) / 4];
const int config_btctler_le_rtx_static_ram = MAX_STATIC_RAM_RTX_BUF;
#else
...
#if SMBOX_MULTI_BLE_EN || FINDMY_EN|| SMBOX_BLE_CLIENT_EN
const int config_btctler_le_master_multilink = 0;//watch 增加为一主一从属于多机行为
const int config_btctler_le_master_or_slave_multilink = 1;
#else
const int config_btctler_le_master_multilink = 0;
const int config_btctler_le_master_or_slave_multilink = 0;
#endif
...
VENDOR_BB_MD_CLOSE |
/* VENDOR_BB_PIS_HB | */
VENDOR_BB_PIS_TX_PAYLOAD_LEN(251) |
VENDOR_BB_RX_PAYLOAD_LEN(251) |
0;
#endif
#else
