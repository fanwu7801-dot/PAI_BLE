...
int ui_page_set_curr_page(int curr_id)
{
    ui_core_redraw(ui_core_get_element_by_id(curr_id));
    return false;
}
...
#if TCFG_CAT1_MODULE_UPDATE_ENABLE
    case DEVICE_EVENT_FROM_CAT1_UPDATE:
        puts("DEVICE_EVENT_FROM_CAT1_UPDATE\n");
        cat1_module_update_task_del(event->u.cfg_tool.event);
        break;
#endif
    case DEVICE_EVENT_FROM_BOTA:
        // extern void bota_dev_event_handler(struct sys_event * evt);
        // bota_dev_event_handler(event);
        break;
    default:
...
void bt_get_time_date()
{
    ...
    u32 error = user_send_cmd_prepare(USER_CTRL_HFP_GET_PHONE_DATE_TIME, 0, NULL);
    log_info(">>>>>error = %d\n", error);
    if (error == 4) {
...
#if 0//USER_SUPPORT_PROFILE_BIP == 1
/*************************************************************************/
//			avrcp传输音乐图片
//参考:uI_action_music_bg.c
/*************************************************************************/
//配置
