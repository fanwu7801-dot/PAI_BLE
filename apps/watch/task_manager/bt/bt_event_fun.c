...
{
    if (!bt_media_is_running()) { // 蓝牙不播歌的时候才清a2dp数据
...
#else
        // bt_ble_init();
        sys_timeout_add(NULL, bt_ble_init, 1000);// 防止ble_init的时候并没有蓝牙栈初始化完成
#endif
    }
#endif
...
#if TCFG_CALL_KWS_SWITCH_ENABLE || TCFG_CALL_IFLYTEK_SWITCH_ENABLE
extern int audio_smart_voice_phone_call_start(void);
extern int audio_smart_voice_phone_call_close(void);
...
/*----------------------------------------------------------------------------*/
/**@brief       蓝牙通话事件离线语音识别相关处理
   @param       event - 通话事件（INCOME, ACTIVE, HANGUP）
   @return      无
   @note
*/
/*----------------------------------------------------------------------------*/
static void jl_call_audio_smart_voice_handler(int event)
{
    if (event == BT_STATUS_PHONE_INCOME) {
        ...
        audio_smart_voice_phone_call_start();
    } else if (event == BT_STATUS_PHONE_ACTIVE) {
        audio_smart_voice_phone_call_close();
        audio_aec_reboot(0);
...
#if TCFG_SMART_VOICE_ENABLE
#if TCFG_CALL_KWS_SWITCH_ENABLE || TCFG_CALL_IFLYTEK_SWITCH_ENABLE
    jl_call_audio_smart_voice_handler(BT_STATUS_PHONE_INCOME);
#endif
#endif
    /*
...
#if TCFG_SMART_VOICE_ENABLE
#if TCFG_CALL_KWS_SWITCH_ENABLE || TCFG_CALL_IFLYTEK_SWITCH_ENABLE
    jl_call_audio_smart_voice_handler(BT_STATUS_PHONE_ACTIVE);
#endif
#endif
...
void bt_set_phone_active_start_time_ms(void)
{
    bt_phone_active_start_time = timer_get_ms();
}
...
#if TCFG_SMART_VOICE_ENABLE
#if TCFG_CALL_KWS_SWITCH_ENABLE || TCFG_CALL_IFLYTEK_SWITCH_ENABLE
    jl_call_audio_smart_voice_handler(BT_STATUS_PHONE_HANGUP);
#endif
#endif
    if (bt_user_priv_var.phone_ring_flag) {
        bt_user_priv_var.phone_ring_flag = 0;
