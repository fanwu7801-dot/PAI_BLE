...
#include "debug.h"
		...
		(TCFG_ADKEY_ENABLE && ((TCFG_ADKEY_PORT==IO_PORT_DP) || (TCFG_ADKE000000000000000Y_PORT==IO_PORT_DM) )) || \
...
LCD_SPI_PLATFORM_DATA_BEGIN(lcd_spi_data)
#if TCFG_LCD_SPI_SH8601A_ENABLE || TCFG_LCD_SPI_ICNA3310B_ENABLE || TCFG_LCD_SPI_SH8501A_ENABLE ||TCFG_LCD_SPI_CO5300_v1_ENABLE
    .pin_reset= IO_PORTA_02,
    .pin_cs	= IO_PORTA_06,
    .pin_bl = TCFG_BACKLIGHT_PWM_IO,
    .pin_dc	= NO_CONFIG_PORT,
    .pin_en	= IO_PORTB_10,
    .pin_te = TCFG_LCD_TE_IO_PORT,
#endif
    ...
    .pin_te = TCFG_LCD_TE_IO_PORT,//IO_PORTA_05,
#endif
...
/* ============ 自定义低功耗控制 ============ */
/* 根据官方文档：注册外设查询空闲函数，控制是否允许进入低功耗 */
u8 can_enter_lp = 1; // 置1可进睡眠，置0不可进睡眠 (默认允许)

static u8 custom_idle_query(void)
{
    return can_enter_lp ? 1 : 0;
}

/**
 * @brief BLE连接状态控制低功耗
 * @param connected: 1=已连接(阻止低功耗), 0=已断开(允许低功耗)
 * 
 * 在BLE连接成功时调用 ble_connection_lowpower_ctrl(1) 阻止进入低功耗
 * 在BLE断开连接时调用 ble_connection_lowpower_ctrl(0) 允许进入低功耗
 */
void ble_connection_lowpower_ctrl(u8 connected)
{
    can_enter_lp = connected ? 0 : 1;
    log_info("BLE %s, can_enter_lp=%d\n", connected ? "CONNECTED" : "DISCONNECTED", can_enter_lp);
}
...
/* ============ 低功耗节点状态调试打印 ============ */
/* 根据官方文档：打印低功耗检测节点状态，全为1才允许进休眠 */
/* 注意：该打印使用 printf(UART) 输出，本身会持续触发 UART 活跃，从而导致 [uart] is_idle 常为 0。
 * 正常版本建议默认关闭；只有在排查低功耗问题时临时打开。
 */
#define LP_TARGET_DEBUG_ENABLE  0

#if LP_TARGET_DEBUG_ENABLE
#include "asm/power/power_api.h"

static void lp_target_printf(void *priv)
{
    struct lp_target *p;
    printf("\n======== LP Target Status ========\n");
...
// UART0 RX唤醒口配置 (IO_PORTA_02)
struct port_wakeup uart0_rx_wakeup_port = {
    .pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
    .edge               = FALLING_EDGE,                      //唤醒方式选择：串口空闲高电平，数据来时下降沿唤醒
    .filter             = PORT_FLT_2ms,                      //2ms滤波
    .iomap              = IO_PORTA_02,                       //UART0 RX引脚
};
...
// 需要双边沿唤醒
// 关闭上下拉，配置两个wkup，分别为上升沿和下降沿
struct port_wakeup port_falling = {
	.edge               = FALLING_EDGE,               		//唤醒方式选择,可选：上升沿\下降沿
	.iomap              = TCFG_CAT1_AICXTEK_GPIO_WKUP,      //唤醒口选择
};
struct port_wakeup port_rising = {
    .pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
...
#include "uos_gpio.h"
struct port_wakeup pin_unisoc_ap_req = {
    .pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
    .edge               = FALLING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .iomap              = TCFG_CAT1_UNISOC_AP_REQ,                    //唤醒口选择
};
struct port_wakeup pin_unisoc_ap_ack = {
    .pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
    .edge               = FALLING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .iomap              = TCFG_CAT1_UNISOC_AP_ACK,                    //唤醒口选择
};
struct port_wakeup pin_unisoc_cp_req = {
    .pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
    .edge               = RISING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .iomap              = TCFG_CAT1_UNISOC_CP_REQ,                    //唤醒口选择
};
struct port_wakeup pin_unisoc_cp_ack = {
    .pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
    .edge               = RISING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .iomap              = TCFG_CAT1_UNISOC_CP_ACK,                    //唤醒口选择
};
struct port_wakeup pin_unisoc_tp_ack = {
    .pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
    .edge               = RISING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .iomap              = TCFG_CAT1_UNISOC_TP_ACK,                    //唤醒口选择
};

struct port_wakeup pin_unisoc_cp_pbint = {
    .pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
    .edge               = RISING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .iomap              = TCFG_CAT1_UNISOC_CP_PBINT,                    //唤醒口选择
};
...
struct port_wakeup te_port = {
    .edge               = BOTH_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿\双边沿
    .filter             = PORT_FLT_4ms,
    .iomap              = TCFG_LCD_TE_IO_PORT,                      //唤醒口选择
};
#endif
...
/* #if (!TCFG_LP_TOUCH_KEY_ENABLE) */
    ...
    // UART0 RX 唤醒口 (IO_PORTA_02)
    .port[6] = &uart0_rx_wakeup_port,
...
#if TCFG_CAT1_AICXTEK_ENABLE
    .port[PORT_WKUP_FALLING_IDX] = &port_falling,
	.port[PORT_WKUP_RISING_IDX]  = &port_rising,
#endif /* #if TCFG_CAT1_AICXTEK_ENABLE */

#if TCFG_CAT1_UNISOC_ENABLE
    .port[PORT_AP_REQ_IDX] = &pin_unisoc_ap_req,
    .port[PORT_AP_ACK_IDX] = &pin_unisoc_ap_ack,
    .port[PORT_CP_REQ_IDX] = &pin_unisoc_cp_req,
    .port[PORT_CP_ACK_IDX] = &pin_unisoc_cp_ack,
    .port[PORT_CP_PBINT_IDX] = &pin_unisoc_cp_pbint,
    //.port[10]   =  &pin_unisoc_tp_ack,
#endif
...
// 触摸屏 和 屏公用复位，先初始化触摸
    ...
    /* ======== UART0 RX唤醒口GPIO初始化 (IO_PORTA_02) ======== */
    /* 根据官方文档：需要自行初始化唤醒口 */
    /* 唤醒方式为下降沿(FALLING_EDGE)，所以设置上拉 */
    gpio_set_pull_down(IO_PORTA_02, 0);   // 禁用下拉
    gpio_set_pull_up(IO_PORTA_02, 1);     // 启用上拉(下降沿唤醒需要)
    gpio_set_die(IO_PORTA_02, 1);         // 数字输入使能
    gpio_set_dieh(IO_PORTA_02, 1);        // 高电平数字输入使能
    gpio_set_direction(IO_PORTA_02, 1);   // 设置为输入
    /* ======== UART0 RX唤醒口GPIO初始化 END ======== */
...
#if TCFG_CAT1_UNISOC_ENABLE
    gpio_set_pull_up(IO_PORTB_01, 0); //展锐样机开机PB1上拉，影响功耗，此处修改为不上拉
#endif
}
...
void board_set_soft_poweroff(void)
{
#if TCFG_APP_CAT1_EN
#if TCFG_CAT1_UNISOC_ENABLE
    gpio_set_direction(TCFG_CAT1_UNISOC_CP_PBINT, 0);
    gpio_direction_output(TCFG_CAT1_UNISOC_CP_PBINT, 0);
    soff_gpio_protect(TCFG_CAT1_UNISOC_CP_PBINT);

    P3_IOV2_CON |= BIT(1);  //展锐样机PB为1.8v，关机将PB恢复为3.3v

    soff_gpio_protect(TCFG_CAT1_UNISOC_TP_ACK);
#elif TCFG_CAT1_AICXTEK_ENABLE
    soff_gpio_protect(TCFG_CAT1_AICXTEK_GPIO_POWER);
#endif
#endif
...
#if TCFG_LOWER_POWEROFF_KEY_DISABLE
	u16 vbat = get_vbat_value();
	if (vbat == 0) {
        vbat = get_vbat_level();
	}
	if ((vbat && (vbat < app_var.poweroff_tone_v)) || adc_check_vbat_lowpower()) {
		gpio_longpress_pin0_reset_config(0, 0, 0);
		key_wakeup_disable();
	} else
#endif /* #if TCFG_LOWER_POWEROFF_KEY_DISABLE */
	{
		key_wakeup_enable();
	}
    ...
    // 保护UART0 RX唤醒口，关机时保留唤醒能力
    soff_gpio_protect(IO_PORTA_02);
...
void sleep_enter_callback(u8  step)
{
    ...
    if (step == 1) {
        ...
        // 保护UART0 RX唤醒口，防止进入高阻态
        sleep_gpio_protect(IO_PORTA_02);
...
#include "uart_update.h"
    ...
    /* 启动低功耗节点状态调试打印 - 调试完毕后删除或设置LP_TARGET_DEBUG_ENABLE为0 */
#if LP_TARGET_DEBUG_ENABLE
    sys_timer_add(NULL, lp_target_printf, 5000);  // 每5秒打印一次低功耗节点状态
    printf("[LP_DEBUG] LP target status timer started\n");
#endif
}
