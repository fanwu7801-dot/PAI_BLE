...
static void book_list_save_all(phonebook_save *save);
static u32 book_list_get_count_in_exflash(u8 type);
static void book_list_save_in_exflash(phonebook_save *save);
...
// 电话号码整理。去空格等
static int phone_number_neaten(const u8 *in_number, u8 *out_number)
{
    int out_len = 0;
...
void set_call_log_call_sel(u8 call_sel)
{
    call_log_mess.sel = call_sel;
    //printf("\n\n\n callsel:%d \n\n\n", call_sel);
}
...
u8 *get_call_log_number(void)
{
    return call_log_mess.number;
}
...
void update_call_log_message()
{
    if (call_log_mess.type != 0) {
        if (strlen(call_log_mess.number)) {
            phonebook_save save = {0};
            save.name = call_log_mess.name;
            save.number = call_log_mess.number;
            save.date = call_log_mess.date;
            save.type = call_log_mess.type;
            save.sel = call_log_mess.sel;
            book_list_save_all(&save);
        }
...
static phonebook *book_list_read_by_index_in_exflash(u8 type, u32 index)//从1开始计数
{
    ...
    phone_number_neaten(phonebook->number, phonebook->number);
...
static void book_list_save_in_exflash(phonebook_save *save)
{
    ...
    message.type = save->type;
    message.sel = save->sel;
    if (save->name) {
        snprintf(message.name, sizeof(message.name), "%s", save->name);
    }

    if (save->date) {
        snprintf(message.date, sizeof(message.date), "%s", save->date);
    } else {
        snprintf(message.date, sizeof(message.date), "%s", "0000/00/00T00:00:00");
    }

    if (save->number) {
        snprintf(message.number, sizeof(message.number), "%s", save->number);
    }
...
int phonebook_get_name_by_number(u8 *in_number, u8 *name)
{
#if TCFG_NOR_VM
...
#else
    book_list_save_in_exflash(save);
#endif
}
