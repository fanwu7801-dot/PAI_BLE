...
// 看门狗管理：用于检测 smartbox 任务卡死（例如 send_data_to_ble 内阻塞/死循环）
#include "wdg_mgr.h"
...
static int wdg_smartbox_task_id = -1;
...
static void app_smartbox_task_start(void)
{
    ...
    if (cat1_module_is_updating()) {
        extern void ancs_client_init_exit(void);
        ancs_client_init_exit();
    }
#endif
...
void app_smartbox_task(void)
{
    ...
    if (wdg_smartbox_task_id <= 0) {
        /* 任务级心跳：只在 smartbox task 运行期间生效，退出时注销 */
        wdg_smartbox_task_id = wdg_mgr_register("smartbox", 6000);
    }
    app_smartbox_task_start();
    while (1) {
        if (wdg_smartbox_task_id > 0) {
            wdg_mgr_heartbeat(wdg_smartbox_task_id);
        }
        ...
        if (app_task_exitting()) {
            ...
            if (wdg_smartbox_task_id > 0) {
                wdg_mgr_unregister(wdg_smartbox_task_id);
                wdg_smartbox_task_id = -1;
            }
            return;
        }
    }
