...
static void common_function_attr_vol_set(void *priv, u8 attr, u8 *data, u16 len)
{
    ...
    if (BT_CALL_HANGUP != call_ctrl_get_status()) {
...
static u32 common_function_attr_dev_info_get(void *priv, u8 attr, u8 *buf, u16 buf_size, u32 offset)
{
...
#if (defined TCFG_NANDFLASH_DEV_ENABLE && TCFG_NANDFLASH_DEV_ENABLE)
    u8 dev_offset = 4;
    struct _DEV_info tmp_dev_info = {0};
    u8 info_len = 6 * sizeof(struct _DEV_info) + 4;
    ...
    if (dev_manager_get_root_path_by_logo("udisk0")) {
        ...
        memset(&tmp_dev_info, 0, sizeof(tmp_dev_info));
        tmp_dev_info.status = 1; // 设备在线
        tmp_dev_info.index = BS_UDISK;
        tmp_dev_info.handle |= app_htonl((u32)BS_UDISK);
        memcpy(dev_info + dev_offset, &tmp_dev_info, sizeof(tmp_dev_info));
        dev_offset += sizeof(tmp_dev_info);
    } else {
        printf("dev [udisk0] offline\n");
        dev_info[dev_offset++] = 0;
        dev_info[dev_offset++] = BS_UDISK;
    }
    ...
    if (dev_manager_get_root_path_by_logo("sd0")) {
        ...
        memset(&tmp_dev_info, 0, sizeof(tmp_dev_info));
        tmp_dev_info.status = 1; // 设备在线
        tmp_dev_info.index = BS_SD0;
        tmp_dev_info.handle |= app_htonl((u32)BS_SD0);
        memcpy(dev_info + dev_offset, &tmp_dev_info, sizeof(tmp_dev_info));
        dev_offset += sizeof(tmp_dev_info);
    } else {
        printf("dev [sd0] offline\n");
        dev_info[dev_offset++] = 0;
        dev_info[dev_offset++] = BS_SD0;
    }
    ...
    if (dev_manager_get_root_path_by_logo("sd1")) {
        ...
        memset(&tmp_dev_info, 0, sizeof(tmp_dev_info));
        tmp_dev_info.status = 1; // 设备在线
        tmp_dev_info.index = BS_SD1;
        tmp_dev_info.handle |= app_htonl((u32)BS_SD1);
        memcpy(dev_info + dev_offset, &tmp_dev_info, sizeof(tmp_dev_info));
        dev_offset += sizeof(tmp_dev_info);
    } else {
        printf("dev [sd1] online\n");
        dev_info[dev_offset++] = 0;
        dev_info[dev_offset++] = BS_SD1;
    }
    ...
    if (smart->file_transfer_mode && dev_manager_get_root_path_by_logo("virfat_flash")) {
        ...
        memset(&tmp_dev_info, 0, sizeof(tmp_dev_info));
        tmp_dev_info.status = 1; // 设备在线
        tmp_dev_info.index = BS_FLASH;
        tmp_dev_info.handle |= app_htonl((u32)BS_FLASH);
        memcpy(dev_info + dev_offset, &tmp_dev_info, sizeof(tmp_dev_info));
        dev_offset += sizeof(tmp_dev_info);
    } else {
        printf("dev [virfat_flash] offline\n");
        dev_info[dev_offset++] = 0;
        dev_info[dev_offset++] = BS_FLASH;
    }
    ...
    if (smart->file_trans_nor_fat && dev_manager_get_root_path_by_logo("fat_nor")) {
        ...
        memset(&tmp_dev_info, 0, sizeof(tmp_dev_info));
        tmp_dev_info.status = 1; // 设备在线
        tmp_dev_info.index = BS_FLASH_2;
        tmp_dev_info.handle |= app_htonl((u32)BS_FLASH_2);
        memcpy(dev_info + dev_offset, &tmp_dev_info, sizeof(tmp_dev_info));
        dev_offset += sizeof(tmp_dev_info);
    } else {
        printf("dev [fat_nor] offline\n");
        dev_info[dev_offset++] = 0;
        dev_info[dev_offset++] = BS_FLASH_2;
    }

#if (defined TCFG_NANDFLASH_FAT_ENABLE && TCFG_NANDFLASH_FAT_ENABLE && (defined TCFG_NANDFLASH_FAT_ROOT))
    if (dev_manager_get_root_path_by_logo(TCFG_NANDFLASH_FAT_ROOT)) {
#else
    if (0) {
#endif
        ...
        memset(&tmp_dev_info, 0, sizeof(tmp_dev_info));
        tmp_dev_info.status = 1; // 设备在线
        tmp_dev_info.index = BS_NAND_FLASH;
        tmp_dev_info.handle |= app_htonl((u32)BS_NAND_FLASH);
        memcpy(dev_info + dev_offset, &tmp_dev_info, sizeof(tmp_dev_info));
        dev_offset += sizeof(tmp_dev_info);
    } else {
        printf("dev [nand_flash] offline\n");
        dev_info[dev_offset++] = 0;
        dev_info[dev_offset++] = BS_NAND_FLASH;
    }
