...
#if (TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_RCSP_DEMO && SMBOX_MULTI_BLE_MASTER_NUMS && SMBOX_BLE_CLIENT_EN)
...
// log switches (force: off) to avoid log spam
// NOTE: Some builds may pass -DLE_CLIENT_*_LOG_EN=1; explicitly override here.
#ifdef LE_CLIENT_MODE_EN_LOG_EN
#undef LE_CLIENT_MODE_EN_LOG_EN
#endif
#define LE_CLIENT_MODE_EN_LOG_EN (0)

#ifdef LE_CLIENT_ADV_NAME_LOG_EN
#undef LE_CLIENT_ADV_NAME_LOG_EN
#endif
#define LE_CLIENT_ADV_NAME_LOG_EN (0)
...
#define TRANS_ANCS_AMS_EN  			  	 1
#if TRANS_ANCS_AMS_EN
#define SUPPORT_MAX_CLIENT          (SMBOX_MULTI_BLE_MASTER_NUMS - 1) 
#if SUPPORT_MAX_CLIENT < 1
#error "support client num "
#endif
#else
...
//连接周期
#define SET_CONN_INTERVAL   (BASE_INTERVAL_MIN*12) //(unit:1.25ms)
//连接latency
#define SET_CONN_LATENCY    0  //(unit:conn_interval)
//连接超时
#define SET_CONN_TIMEOUT    600 //(unit:10ms)
...
//指定索索名字
static const u8 test_remoter_name1[] = "JL707_watch(BLE)";//
...
static const client_conn_cfg_t client_conn_config = {
    ...
    .search_uuid_cnt =0,// (sizeof(jl_search_uuid_table) / sizeof(target_uuid_t)),
...
static void client_report_adv_data(adv_report_t *report_pt, u16 len)
{
    ...
    /* 输出扫描日志：把 ADV 上报交给 per_key_scan 解析(名称/MAC/RSSI) */
    per_key_scan_on_adv_report_ex(report_pt->address, report_pt->data, report_pt->length, report_pt->rssi);
...
#ifndef LE_CLIENT_PACKET_DEBUG_LOG_EN
#define LE_CLIENT_PACKET_DEBUG_LOG_EN (0)
#endif

#if LE_CLIENT_PACKET_DEBUG_LOG_EN
    log_info("cbk_packet_handler %0x\n", packet_type);
#endif
    switch (packet_type) {
    case HCI_EVENT_PACKET:
#if LE_CLIENT_PACKET_DEBUG_LOG_EN
        log_info("HCI_EVENT_PACKET %0x\n", hci_event_packet_get_type(packet));
#endif
        switch (hci_event_packet_get_type(packet)) {
...
void client_profile_init(void)
{
    ...
    /* per_key_scan：
     * 仅接入 ADV 上报解析并不足以看到日志：如果当前并未启用扫描，就不会有任何 ADV report。
     * 这里在初始化后主动触发 per_key_scan 的扫描窗口，确保能看到周边设备 MAC/Name/RSSI。
     */
    per_key_scan_init();
    per_key_scan_enable(1);
    per_key_scan_mac();
}
