...
#if 0//((TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_RCSP_DEMO) && SMBOX_MULTI_BLE_SLAVE_NUMS && SMBOX_MULTI_BLE_EN)
...
u32 smbox_pairing_generate_passkey(uint8_t code3[3])
{
    u32 key = rand32();
    key &= 0xfffff;
    key %= 1000000;
...
void smbox_pairing_set_pending(u16 conn_handle, const uint8_t code3[3], u32 passkey)
{
    g_pair_pending_handle = conn_handle;
...
u32 smbox_pairing_get_pending_passkey(u16 conn_handle)
{
    if (g_pair_pending_valid && g_pair_pending_handle == conn_handle && g_pair_pending_passkey) {
        return g_pair_pending_passkey;
    }
    return 0;
}

static void smbox_pairing_clear_pending(void)
{
    g_pair_pending_valid = 0;
    g_pair_pending_handle = 0;
...
static void smbox_pairing_save_code_and_mac(void)
{
    ...
    memcpy(record + 3, id_addr, sizeof(id_addr));
    ...
    int chosen = -1;
    for (u16 i = 0; i < (sizeof(config_ids) / sizeof(config_ids[0])); i++) {
        u8 tmp[9] = {0};
        ...
        if (chosen < 0 && (r <= 0 || is_all_zero_local(tmp, sizeof(tmp)))) {
            chosen = (int)i;
        }
    }

    if (chosen < 0) {
        chosen = 0; /* full: overwrite slot1 */
    }
    ...
    for (u16 i = 0; i < (sizeof(config_ids) / sizeof(config_ids[0])); i++) {
        u8 tmp[9] = {0};
        int r = syscfg_read(config_ids[i], tmp, sizeof(tmp));
        if (r > 0 && !is_all_zero_local(tmp, sizeof(tmp))) {
            count++;
        }
    }
    syscfg_write(CFG_PHONE_BLE_KEY_USABLE_NUM, &count, sizeof(count));

    log_info("pair save ok: slot=%d passcode=%02u%02u%02u\n", chosen,
             g_pair_pending_code3[0], g_pair_pending_code3[1], g_pair_pending_code3[2]);
}
