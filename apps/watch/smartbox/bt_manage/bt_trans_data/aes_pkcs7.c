#include "aes_pkcs7.h"

#define LOG_TAG "[aes]"

#define LOG_ERROR_ENABLE
#define LOG_INFO_ENABLE
#define LOG_DEBUG_ENABLE

#include "debug.h"

#define GETU32(p)                                                              \
  ((uint32_t)(p)[0] << 24 | (uint32_t)(p)[1] << 16 | (uint32_t)(p)[2] << 8 |   \
   (uint32_t)(p)[3])

#define ROL32(a, n) (((a) << (n)) | (((a) & 0xffffffff) >> (32 - (n))))

static const uint8_t S[256] = {
    ,
};

static const uint8_t S_inv[256] = {
    ,
};

static const uint8_t Rcon[11] = {
    0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
};

/* (a0,a1,a2,a3) => (a1,a2,a3,a0) */
static uint32_t rot_word(uint32_t A) { return ROL32(A, 8); }

static uint32_t sub_word(uint32_t A) {
  return S[(A >> 24) & 0xff] << 24 | S[(A >> 16) & 0xff] << 16 |
         S[(A >> 8) & 0xff] << 8 | S[A & 0xff];
}

/*
 * |S00 S01 S02 S03|     |           |
 * |S10 S11 S12 S13| xor |W0 W1 W2 W3|
 * |S20 S21 S22 S23|     |           |
 * |S30 S31 S32 S33|     |           |
 */
static void add_round_key(uint8_t state[4][4], const uint32_t *W) {
  int i;
  for (i = 0; i < 4; i++) {
    state[0][i] ^= (W[i] >> 24) & 0xff;
    state[1][i] ^= (W[i] >> 16) & 0xff;
    state[2][i] ^= (W[i] >> 8) & 0xff;
    state[3][i] ^= (W[i]) & 0xff;
  }
}

static void sub_bytes(uint8_t state[4][4]) {
  int i, j;
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      state[i][j] = S[state[i][j]];
    }
  }
}

static void inv_sub_bytes(uint8_t state[4][4]) {
  int i, j;
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      state[i][j] = S_inv[state[i][j]];
    }
  }
}

/*
 * |S00 S01 S02 S03| <<<0     |S00 S01 S02 S03|
 * |S10 S11 S12 S13| <<<1 =>  |S11 S12 S13 S10|
 * |S20 S21 S22 S23| <<<2     |S22 S23 S20 S21|
 * |S30 S31 S32 S33| <<<3     |S33 S30 S31 S32|
 */
static void shift_rows(uint8_t state[4][4]) {
  uint8_t tmp[4][4];

  tmp[0][0] = state[0][0];
  tmp[0][1] = state[0][1];
  tmp[0][2] = state[0][2];
  tmp[0][3] = state[0][3];

  tmp[1][0] = state[1][1];
  tmp[1][1] = state[1][2];
  tmp[1][2] = state[1][3];
  tmp[1][3] = state[1][0];

  tmp[2][0] = state[2][2];
  tmp[2][1] = state[2][3];
  tmp[2][2] = state[2][0];
  tmp[2][3] = state[2][1];

  tmp[3][0] = state[3][3];
  tmp[3][1] = state[3][0];
  tmp[3][2] = state[3][1];
  tmp[3][3] = state[3][2];

  memcpy(state, tmp, sizeof(tmp));
  memset(tmp, 0, sizeof(tmp));
}

/*
 * |S00 S01 S02 S03| >>>0     |S00 S01 S02 S03|
 * |S10 S11 S12 S13| >>>1 =>  |S13 S10 S11 S12|
 * |S20 S21 S22 S23| >>>2     |S22 S23 S20 S21|
 * |S30 S31 S32 S33| >>>3     |S31 S32 S33 S30|
 */
static void inv_shift_rows(uint8_t state[4][4]) {
  uint8_t tmp[4][4];

  tmp[0][0] = state[0][0];
  tmp[0][1] = state[0][1];
  tmp[0][2] = state[0][2];
  tmp[0][3] = state[0][3];

  tmp[1][0] = state[1][3];
  tmp[1][1] = state[1][0];
  tmp[1][2] = state[1][1];
  tmp[1][3] = state[1][2];

  tmp[2][0] = state[2][2];
  tmp[2][1] = state[2][3];
  tmp[2][2] = state[2][0];
  tmp[2][3] = state[2][1];

  tmp[3][0] = state[3][1];
  tmp[3][1] = state[3][2];
  tmp[3][2] = state[3][3];
  tmp[3][3] = state[3][0];

  memcpy(state, tmp, sizeof(tmp));
  memset(tmp, 0, sizeof(tmp));
}

/*
 * GF(2^8) defSed by f(x) = x^8 + x^4 + x^3 + x + 1
 * x^8 == x^4 + x^3 + x + 1 = 0001,1011 = 0x1b
 * if A[7] == 0 then 2 * A = (A << 1)
 *              else 2 * A = (A << 1) xor A
 */
#define x1(a) (a)

static uint8_t x2(uint8_t a) { return (a >> 7) ? ((a << 1) ^ 0x1b) : (a << 1); }

static uint8_t x3(uint8_t a) { return x2(a) ^ x1(a); }

static uint8_t x9(uint8_t a) { return x2(x2(x2(a))) ^ x1(a); }

/* 0x0b = 11 = 8 + 2 + 1 */
static uint8_t xb(uint8_t a) { return x2(x2(x2(a))) ^ x2(a) ^ x1(a); }

/* 0x0d = 13 = 8 + 4 + 1 */
static uint8_t xd(uint8_t a) { return x2(x2(x2(a))) ^ x2(x2(a)) ^ x1(a); }

/* 0x0e = 14 = 8 + 4 + 2 */
static uint8_t xe(uint8_t a) { return x2(x2(x2(a))) ^ x2(x2(a)) ^ x2(a); }

/*
 * |2  3  1  1| |S00 S01 S02 S03|
 * |1  2  3  1| |S10 S11 S12 S13|
 * |1  1  2  3|*|S20 S21 S22 S23|
 * |3  1  1  2| |S30 S31 S32 S33|
 */
static void mix_columns(uint8_t S[4][4]) {
  uint8_t tmp[4][4];
  int i;

  /* i-th column */
  for (i = 0; i < 4; i++) {
    tmp[0][i] = x2(S[0][i]) ^ x3(S[1][i]) ^ x1(S[2][i]) ^ x1(S[3][i]);
    tmp[1][i] = x1(S[0][i]) ^ x2(S[1][i]) ^ x3(S[2][i]) ^ x1(S[3][i]);
    tmp[2][i] = x1(S[0][i]) ^ x1(S[1][i]) ^ x2(S[2][i]) ^ x3(S[3][i]);
    tmp[3][i] = x3(S[0][i]) ^ x1(S[1][i]) ^ x1(S[2][i]) ^ x2(S[3][i]);
  }

  memcpy(S, tmp, sizeof(tmp));
  memset(tmp, 0, sizeof(tmp));
}

/*
 * |0E 0B 0D 09| |02 03 01 01|   |1  0  0  0|
 * |09 0E 0B 0D|*|01 02 03 01| = |0  1  0  0|
 * |0D 09 0E 0B| |01 01 02 03|   |0  0  1  0|
 * |0B 0D 09 0E| |03 01 01 02|   |0  0  0  1|
 *
 */
static void inv_mix_columns(uint8_t S[4][4]) {
  uint8_t tmp[4][4];
  int i;

  /* i-th column */
  for (i = 0; i < 4; i++) {
    tmp[0][i] = xe(S[0][i]) ^ xb(S[1][i]) ^ xd(S[2][i]) ^ x9(S[3][i]);
    tmp[1][i] = x9(S[0][i]) ^ xe(S[1][i]) ^ xb(S[2][i]) ^ xd(S[3][i]);
    tmp[2][i] = xd(S[0][i]) ^ x9(S[1][i]) ^ xe(S[2][i]) ^ xb(S[3][i]);
    tmp[3][i] = xb(S[0][i]) ^ xd(S[1][i]) ^ x9(S[2][i]) ^ xe(S[3][i]);
  }

  memcpy(S, tmp, sizeof(tmp));
  memset(tmp, 0, sizeof(tmp));
}

int aes_set_encrypt_key(AES_KEY *aes_key, const uint8_t *key, size_t keylen) {
  /* Nk: num user key words
   * AES-128	Nk = 4		W[44]
   * AES-192	Nk = 6		W[52]
   * AES-256	Nk = 8		W[60]
   */
  uint32_t *W = (uint32_t *)aes_key->rk;
  size_t Nk = keylen / sizeof(uint32_t);
  size_t i;

  switch (keylen) {
  case AES128_KEY_SIZE:
    aes_key->rounds = 10;
    break;
  case AES192_KEY_SIZE:
    aes_key->rounds = 12;
    break;
  case AES256_KEY_SIZE:
    aes_key->rounds = 14;
    break;
  default:
    return 0;
  }

  for (i = 0; i < Nk; i++) {
    W[i] = GETU32(key + sizeof(uint32_t) * i);
  }
  for (; i < 4 * (aes_key->rounds + 1); i++) {
    uint32_t T = W[i - 1];
    if (i % Nk == 0) {
      T = rot_word(T);
      T = sub_word(T);
      T ^= ((uint32_t)Rcon[i / Nk] << 24);

    } else if (Nk == 8 && i % 8 == 4) {
      T = sub_word(T);
    }
    W[i] = W[i - Nk] ^ T;
  }

#if 0
    log_info("aes_key->rk %p size %d rounds %d",aes_key->rk,sizeof(aes_key->rk),aes_key->rounds);
    log_debug_hexdump(aes_key->rk, sizeof(aes_key->rk));
#endif

  return 1;
}

int aes_set_decrypt_key(AES_KEY *aes_key, const uint8_t *key, size_t keylen) {
  int ret = 0;
  AES_KEY enc_key;
  size_t i;

  if (!aes_set_encrypt_key(&enc_key, key, keylen)) {
    goto end;
  }

  for (i = 0; i <= enc_key.rounds; i++) {
    aes_key->rk[4 * i] = enc_key.rk[4 * (enc_key.rounds - i)];
    aes_key->rk[4 * i + 1] = enc_key.rk[4 * (enc_key.rounds - i) + 1];
    aes_key->rk[4 * i + 2] = enc_key.rk[4 * (enc_key.rounds - i) + 2];
    aes_key->rk[4 * i + 3] = enc_key.rk[4 * (enc_key.rounds - i) + 3];
  }
  aes_key->rounds = enc_key.rounds;
  ret = 1;

#ifdef CRYPTO_INFO
  print_rk(aes_key);
#endif

end:
  memset(&enc_key, 0, sizeof(AES_KEY));
  return ret;
}

void aes_encrypt(const AES_KEY *key, const uint8_t in[16], uint8_t out[16]) {
  uint8_t state[4][4];
  size_t i;

  /* fill state columns */
  for (i = 0; i < 4; i++) {
    state[0][i] = *in++;
    state[1][i] = *in++;
    state[2][i] = *in++;
    state[3][i] = *in++;
  }

  /* Sitial add round key */
  add_round_key(state, key->rk);

  /* first n-1 rounds */
  for (i = 1; i < key->rounds; i++) {
    sub_bytes(state);
    shift_rows(state);
    mix_columns(state);
    add_round_key(state, key->rk + 4 * i);
  }

  /* last round withtmp mix columns */
  sub_bytes(state);
  shift_rows(state);
  add_round_key(state, key->rk + 4 * i);

  /* tmpput state columns */
  for (i = 0; i < 4; i++) {
    *out++ = state[0][i];
    *out++ = state[1][i];
    *out++ = state[2][i];
    *out++ = state[3][i];
  }

  memset(state, 0, sizeof(state));
}

void aes_decrypt(const AES_KEY *aes_key, const uint8_t in[16],
                 uint8_t out[16]) {
  uint8_t state[4][4];
  size_t i;

  /* fill state columns */
  for (i = 0; i < 4; i++) {
    state[0][i] = *in++;
    state[1][i] = *in++;
    state[2][i] = *in++;
    state[3][i] = *in++;
  }

  /* Sitial add round key */
  add_round_key(state, aes_key->rk);

  /* first n-1 rounds */
  for (i = 1; i < aes_key->rounds; i++) {
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, aes_key->rk + 4 * i);
    inv_mix_columns(state);
  }

  /* last round withtmp mix columns */
  inv_shift_rows(state);
  inv_sub_bytes(state);
  add_round_key(state, aes_key->rk + 4 * i);

  /* tmpput state columns */
  for (i = 0; i < 4; i++) {
    *out++ = state[0][i];
    *out++ = state[1][i];
    *out++ = state[2][i];
    *out++ = state[3][i];
  }

  memset(state, 0, sizeof(state));
}

uint8_t aes_decrypt_pkcs(const AES_KEY *aes_key, uint8_t *in, uint16_t in_len,
                         uint8_t *out, uint16_t *out_len) {

  if (in_len % 16 != 0) {
    log_error("in_len %d is error", in_len);
    return -1;
  }

  uint16_t group_16 = in_len / 16;
  uint16_t temp_out_len = 0;
  uint8_t padd_num = 0;
  // log_info("group_16 %d padd_num %d ",group_16,padd_num);
  for (uint16_t i = 0; i < group_16; i++) {
    aes_decrypt(aes_key, &in[i * 16], &out[i * 16]);
    temp_out_len = temp_out_len + 16;
  }

  padd_num = out[temp_out_len - 1];
  if (padd_num > 16) {
    log_error("padd_num %d is error", padd_num);
    return -1;
  }

  temp_out_len = temp_out_len - padd_num;
  // log_info("temp_out_len %d",temp_out_len);

  for (uint8_t j = 0; j < padd_num; j++) {
    out[temp_out_len + j] = 0;
    // log_info("out[%d] %d",temp_out_len + j,out[temp_out_len + j]);
  }
  // log_info("out-------------%d",temp_out_len);
  // log_debug_hexdump(out,temp_out_len);
  *out_len = temp_out_len;
  return 0;
}

uint16_t aes_encrypt_pkcs(const AES_KEY *aes_key, uint8_t *in, uint16_t in_len,
                          uint8_t *out, uint16_t *out_len) {

  uint8_t flag = 0;
  uint16_t temp_out_len = 0;
  uint16_t group_16 = in_len / 16;
  uint16_t mode_16 = in_len % 16;
  uint8_t padd_num = 16 - mode_16;
  // if(in_len%16 != 0)
  //     flag=1;

  // uint16_t aes_times = group_16 + (flag ==1?1:0);
  uint16_t aes_times = group_16 + 1;
  // log_info("group_16 %d padd_num %d mode_16 %d aes_times
  // %d",group_16,padd_num,mode_16,aes_times);
  for (uint16_t i = 0; i < aes_times; i++) {

    if (i == aes_times - 1) {
      uint8_t temp[16] = {0};
      memset(temp, 0, sizeof(temp));
      memcpy(temp, &in[i * 16], mode_16);

      for (uint8_t j = 0; j < padd_num; j++) {
        temp[mode_16 + j] = padd_num;
        // log_info("j %d padd_num %d mode_16 %d temp
        // %d",j,padd_num,mode_16,temp[mode_16 +j]);
      }

      aes_encrypt(aes_key, temp, &out[i * 16]);
      temp_out_len += 16;
      // log_debug_hexdump(temp,16);
      // log_info("out-------------%d",i);
      // log_debug_hexdump(&out[i*16],16);

      break;
    }

    aes_encrypt(aes_key, &in[i * 16], &out[i * 16]);
    // log_info("in------------- %d",i);
    // log_debug_hexdump(&in[i*16],16);
    // log_info("out-------------%d",i);
    // log_debug_hexdump(&out[i*16],16);

    temp_out_len += 16;
  }
  *out_len = temp_out_len;

  return 0;
}
