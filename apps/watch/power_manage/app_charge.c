...
extern int get_need_trim_flag();
extern void watch_reboot_or_shutdown(u8 flag, u8 erase);
...
void charge_ldo5v_in_deal(void)
{
    ...
    charge_full_flag = 0;
    log_info("ldo5v_in: poweron_en=%d app=%d online=%d ldo5v=%d goto_off=%d",
             get_charge_poweron_en(), app_get_curr_task(),
             get_charge_online_flag(), get_ldo5v_online_hw(), app_var.goto_poweroff_flag);
...
_check_reset:
    ...
    if (app_var.goto_poweroff_flag) {
        /* 充电插入时避免因 goto_poweroff_flag 导致复位，保持广播/音频链路 */
        if (get_charge_online_flag() || get_ldo5v_online_hw()) {
...
#if 1
        /*
        	ota后 开机vbat电压低于3.7v时,不能进行power trim
        	此时功耗偏高，耗电速度快

        	可通过提高允许ota的电量(smart->low_battery_level)，避免出现此情况
        	或在ota后充电拔出电压高于3.7v时 复位手表
         */
        if (get_need_trim_flag()) {
            u32 check_vbat = adc_get_voltage_preemption(AD_CH_VBAT) * 4;
            if (check_vbat > 3700) {
                printf("[warning]need_trim_flag is %d,curr vbat: %d mv,please reboot the watch\n", get_need_trim_flag(), check_vbat);
                /*下为复位流程参考*/
...
#if (TCFG_APP_PC_EN && (!TCFG_OTG_MODE))
        if (pc_app_check()) {
            UI_WINDOW_PREEMPTION_POSH(ID_WINDOW_PC, NULL, NULL, UI_WINDOW_PREEMPTION_TYPE_CHARGE);
        }
#endif /* #if (TCFG_APP_PC_EN || (!TCFG_OTG_MODE)) */
