...
int app_power_event_handler(struct device_event *dev)
{
...
#if GFPS_EN
    extern int gfps_app_power_event_handler(u8 event);
    gfps_app_power_event_handler(dev->event);
#endif
    ...
    case POWER_EVENT_POWER_CHANGE:
        ...
        user_send_cmd_prepare(USER_CTRL_HFP_CMD_UPDATE_BATTARY, 0, NULL);
#endif
        SMARTBOX_UPDATE(COMMON_FUNCTION, BIT(COMMON_FUNCTION_ATTR_TYPE_BATTERY));
        break;
    case POWER_EVENT_POWER_CHARGE:
        ...
        /* 充电时强制保持提示音音量，避免被静音 */
        {
            s8 vol = get_ui_sys_param(LastSysVol);
            if (vol <= 0) {
                vol = get_tone_vol();
            }
            app_audio_set_volume(APP_AUDIO_STATE_WTONE, vol, 1);
        }
        break;
    default:
        break;
...
void vbat_check_slow(void *priv)
{
    if (vbat_fast_timer == 0) {
        vbat_fast_timer = usr_timer_add(NULL, vbat_check, 10, 0);
    }
    if (get_charge_online_flag()) {
...
{
    static u8 is_init_vbat_check = 0;

    if (is_init_vbat_check == 1) {
        return;
    }
    is_init_vbat_check = 1;

    // 快速执行两次，建立vbat初值
    vbat_check(NULL);
    usr_timeout_add(NULL, vbat_check, 50, 0);

    sys_timer_add(NULL, vbat_check, 3600 * 1000);
    usr_timer_add(NULL, vbat_check, 10 * 1000, 0);
    return;
