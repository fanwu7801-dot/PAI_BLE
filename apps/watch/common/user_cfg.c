...
void cfg_file_parse(u8 idx)
{
    ...
    if (ret > 0) {
        ...
        log_info_hexdump((u8 *)&aec, sizeof(aec));
...
#else
    {
        ...
        log_info("boot read CFG_MUSIC_VOL ret=%d val=%d", ret, music_volume);
        if (ret < 0) {
            music_volume = -1;
        }
#endif
        ...
        app_var.music_volume = music_volume <= 0 ? default_volume : music_volume;
#endif
        /* 音效音量与音乐音量保持一致，确保关机/开机后提示音不回落到默认值 */
    ...
    if (ret != 6 || !memcmp(mac_buf, mac_buf_tmp, 6))
        ...
        do {
            ...
            log_info("[MAC_DIAG] syscfg_read ret=%d, mac_buf:", ret);
            log_info_hexdump(mac_buf, 6);
            if ((ret != 6) || !memcmp(mac_buf, mac_buf_tmp, 6) || !memcmp(mac_buf, mac_buf_tmp2, 6)) {
                // syscfg 读取失败，使用基于芯片唯一ID生成的固定 MAC
                u16 chip_id = get_chip_id();
                // 用 chip_id 和固定前缀生成 MAC 地址，确保每个设备唯一且固定
                mac_buf[0] = 0x50;  // PD (PaiDian) 前缀
                mac_buf[1] = 0x44;
                mac_buf[2] = 0x01;
                mac_buf[3] = 0x00;
                mac_buf[4] = (chip_id >> 8) & 0xFF;
                mac_buf[5] = chip_id & 0xFF;
                syscfg_write(CFG_BT_MAC_ADDR, mac_buf, 6);
                log_info("[MAC_DIAG] >>>MAC generated from chip_id(0x%04X) and saved!!!\n", chip_id);
                log_info_hexdump(mac_buf, 6);
            } else {
                log_info("[MAC_DIAG] >>>Using existing MAC from flash\n");
            }
        } while (0);
