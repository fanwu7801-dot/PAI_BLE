...
//rgb
void lcd_rgb_start_refresh();
void lcd_rgb_stop_refresh();
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief imd_custom_clk_set 供客户自定义发命令和发数据独立IMD时钟,没有该需求不用调此接口
 *
 * @param cmd_clock MHZ, 期望的发命令时钟, 必须是大于0的值
 * @param dat_clock MHZ, 期望的发数据时钟, 必须是大于0的值
 * @param dat_clock_default 如果用dat_clock的值作为发数据时钟,dat_clock_default写0,
 * @如果默认用屏幕帧率计算得到的值作为发数据时钟,则dat_clock_default写1
 * @注:该函数应放在lcd(imd)初始化完成之后
 */
/* ------------------------------------------------------------------------------------*/
void imd_custom_clk_set(int cmd_clock, int dat_clock, int dat_clock_default);

/* ------------------------------------------------------------------------------------*/
/* @brief 库里面调用,应用层不调用 */
/* ------------------------------------------------------------------------------------*/
void imd_cmd_clk_div();
void imd_dat_clk_div();
