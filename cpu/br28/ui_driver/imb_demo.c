...
extern int imb_image_init(struct imb_task *p);
extern int imb_image_uninit(struct imb_task *p);
extern const int UI_DATA_STORE_IN_NORFLASH;
extern int ui_gif_msg_type_get();
extern void wdt_clr(void);
extern int ui_core_pagemove_flag();
extern void gif_play(int root_index, struct imb_task *task, int elm_id);
extern int imb_task_delete(struct imb_task_head *root, struct imb_task *task);
...
#define JPEG_SD_TEST 						0//jpeg从sd卡读取数据解码,只能有一个task使用这种从sd卡读取的方式！！！注意，从SD卡读取数据解码，比直接指向地址的方式会慢一些！！
#define IMB_GIF_TEST                        1 //gif动画显示
//<可多选>
#define NANDFLASH_TEST						0 //nandflash测试
#define IMAGE_DECOMPRESS_TEST               0 //图像解压缩
#define IMAGE_TEST                         	0 //图片显示
#define IMAGE_SCALE_TEST                    1 //图片缩放显示
#define IMAGE_ROTATE_TEST                   1 //图片旋转显示
...
/*
    nandflash说明
	1.2d硬件不能直接访问nandflash，需要将资源加载到sram或psram
	2.此demo提供imb_image_init/uninit的方式加载资源到psram或sram(优先到psram)和释放资源
	3.也可定义函数将资源数据读取到psram或sram等介质 再配置将数据地址配置给imb任务
	4.不建议分块旋转
 */
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief imb_demo_enter_cb nandflash加载资源使用
 *
 * @param p
 *
 * @return
 */
/* ------------------------------------------------------------------------------------*/
static void *imb_demo_enter_cb(struct imb_task *p)
{
    imb_image_init(p);
    return NULL;
}
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief imb_demo_exit_cb nandflash 释放资源使用
 *
 * @param p
 *
 * @return
 */
/* ------------------------------------------------------------------------------------*/
static void *imb_demo_exit_cb(struct imb_task *p)
{
    imb_image_uninit(p);
    return NULL;
}
...
void imb_create_image_task(int task_id, struct image_file *file, u32 priv, int x, int y, int width, int height, struct rect *crop, int zip, int load_method)
{
    ...
    ASSERT(!((load_method == LOAD_FROM_FLASH_WITHOUT_TAB) && (!UI_DATA_STORE_IN_NORFLASH)));
    ...
    if (load_method == LOAD_FROM_FLASH_WITH_TAB) {
        if (UI_DATA_STORE_IN_NORFLASH) {
            ...
            imb_tsk_info.use_filesystem = 1;
            imb_tsk_info.file_offset    = file->offset;
            imb_tsk_info.file_len    	= file->len;
            imb_tsk_info.file   		= priv;
            imb_tsk_info.cb             = imb_demo_enter_cb;
            imb_tsk_info.outcb          = imb_demo_exit_cb;
        }
    } else {
        imb_tsk_info.data_src = DATA_SRC_SRAM;
        imb_tsk_info.cur_in_flash = 0;
...
static int post_imb_msg(int *msg, u8 len)
{
    int err = 0;
    int count = 0;

__retry:
    err =  os_taskq_post_type(IMB_TASK_NAME, msg[0], len - 1, &msg[1]);
    ...
    if (err) {
        if (!strcmp(os_current_task(), IMB_TASK_NAME)) {
            return err;
        }
        if (count > 20) {
            return -1;
...
static int imb_gif_play_post(struct imb_task *task, u32 msec, u32 delay, u32 elm_id, u32 root_index)
{
    u32 rets;
    ...
    struct gif_info *gif_info = gif_info_get(task);
...
static void *imb_gif_timer(void *priv)
{
    struct imb_task *task = (struct imb_task *)priv;
    struct gif_info *gif_info = gif_info_get(task);

    // printf("imb_gif_timer\n");

    imb_gif_play_post(task, jiffies_msec(), gif_info->gif_delay, task->elm_id, task->root_index);
    gif_info->gif_frame_index++;
    if (gif_info->gif_frame_index > gif_info->gif_frame_num) {
        gif_info->gif_frame_index = 1;
    }
    /* printf("%s gif frame %d/%d\n", task->name, gif_info->gif_frame_index, gif_info->gif_frame_num); */

    return NULL;
}


static void *imb_gif_enter_cb(void *priv)
{
    struct imb_task *task = (struct imb_task *)priv;
    struct imb_task_head *root = imb_task_head_get(task->root_index);

    struct gif_info *gif_info = gif_info_get(task);
    if (gif_info->task) {
        if (gif_info->gif_type == 0) {
            imb_task_enable(gif_info->task, false); //改task_en可能无效，不会同步到cur_task_valid
            gif_info->task->cur_tsk_valid = false;
        }
    }
...
static void *imb_gif_exit_cb(void *priv)
{
    struct imb_task *task = (struct imb_task *)priv;
    struct imb_task_head *root = imb_task_head_get(task->root_index);
...
int imb_task_delete_enter_callback(struct imb_task_head *root, struct imb_task *task)//函数重载, delete imb_task时调用
{
    if (task->cb == imb_gif_enter_cb) {
        extern int ui_gif_msg_type_get();
        gif_decode_uninit(task, IMB_TASK_NAME, ui_gif_msg_type_get);
    }
...
*/
/*----------------------------------------------------------------------------*/
struct imb_task *imb_create_gif_task(int task_id, void *fp, struct image_file *file, u32 priv, int (*read)(void *fp, int offset, u8 *data, int len), int x, int y, int width, int height)
{
    struct imb_task_info imb_tsk_info = {0};
    struct flash_file_info *file_info = (struct flash_file_info *)priv;
    struct image_file gif_image_file = {0};
    struct imb_task *p = NULL;
    ...
    struct gif_file gif_file = {0};
    if (p) {
        struct gif_info *gif_info = gif_info_get(p);
        gif_frame_info.addr = gif_info->gif_addr;
        gif_frame_info.size = gif_info->gif_size;
        gif_frame_info.type = gif_info->gif_type;
        gif_frame_info.compress = gif_info->gif_compress;
        gif_frame_info.buf = gif_info->gif_buf;
    } else {
        ASSERT(sizeof(struct gif_info) <= sizeof(p->priv_buf), ", sizeof(struct gif_info) = %d", sizeof(struct gif_info));
        gif_file.fp = fp;
        gif_file.offset = file->offset;
        gif_file.len = file->len;
        gif_file.file_info = file_info;
        gif_file.extern_file_info = file_info ? 1 : 0;
        gif_file.read = read;
        gif_read_head_info(&gif_file, &gif_file_info, &gif_frame_info);
        printf("[gif file info]  %d x %d, frame : %d, delay : %d\n", gif_file_info.width, gif_file_info.height, gif_file_info.frame_num, gif_file_info.delay);
        printf("[gif frame info]  addr : 0x%x, size : 0x%x, type : %d, compress : %d, delay : %d\n", gif_frame_info.addr, gif_frame_info.size, gif_frame_info.type, gif_frame_info.compress, gif_frame_info.delay);
        if (!gif_file_info.delay) {
            gif_file_info.delay = 100;
        }

        gif_image_file.offset = gif_file.offset + gif_frame_info.addr;
        gif_image_file.len = gif_frame_info.size;
        gif_image_file.compress = gif_frame_info.compress;
    }

    if (gif_image_file.compress == COMPRESS_LOW) {
        if (gif_file.file_info) {//LOAD_FROM_FLASH_WITH_TAB
            imb_tsk_info.data_src       = DATA_SRC_FLASH;
            imb_tsk_info.cur_in_flash   = 1;
            imb_tsk_info.addr_source    = SOURCE_DEFAULT;
        } else {
            imb_tsk_info.data_src       = DATA_SRC_SRAM;
            if ((gif_frame_info.buf >= 0x2000000) && (gif_frame_info.buf < 0x3000000)) {
                imb_tsk_info.addr_source    = SOURCE_PSRAM;
            } else {
                imb_tsk_info.addr_source    = SOURCE_DEFAULT;
            }
        ...
        imb_tsk_info.has_lut = 1;
    } else if (gif_image_file.compress == COMPRESS_MIDDLE) {
        imb_tsk_info.data_src       = DATA_SRC_SRAM;
        imb_tsk_info.cur_in_flash   = 0;
        imb_tsk_info.zip_en         = 1;
        if ((gif_frame_info.buf >= 0x2000000) && (gif_frame_info.buf < 0x3000000)) {
    ...
    } else if (gif_image_file.compress == COMPRESS_HIGH) {
        imb_tsk_info.data_src       = DATA_SRC_SRAM;
        imb_tsk_info.cur_in_flash   = 0;
        imb_tsk_info.zip_en         = 0;
        if ((gif_frame_info.buf >= 0x2000000) && (gif_frame_info.buf < 0x3000000)) {
    ...
    imb_tsk_info.in_format      = LAYER_FORMAT_L8;
    imb_tsk_info.cb             = imb_gif_enter_cb;
    imb_tsk_info.outcb          = imb_gif_exit_cb;
    imb_tsk_info.datalen        = gif_file.len;
    imb_tsk_info.ff_info        = gif_file.file_info;
    if ((imb_tsk_info.data_src == DATA_SRC_SRAM) || (imb_tsk_info.data_src == DATA_SRC_PSRAM)) {
        imb_tsk_info.priv   = gif_frame_info.buf;
    } else if (imb_tsk_info.data_src == DATA_SRC_FLASH) {
        imb_tsk_info.priv       = (u32)&gif_image_file;
    } else {
        ASSERT(0, "error\n");
    }
    ...
    if (p) {
        ...
        gif_decode_init(&gif_file, &gif_file_info, &gif_frame_info, p, imb_gif_timer);
    }
...
void imb_create_jpeg_task(int task_id, struct image_file *file, u32 priv, int lcd_format, int x, int y, int width, int height, int load_method)
{
    struct imb_task_info imb_tsk_info = {0};
    struct imb_task *p = NULL;
    struct jpeg_input_info *__jpg_info = NULL;
    ...
    if (load_method == LOAD_FROM_FLASH_WITH_TAB) {
        ...
        __jpg_info = (struct jpeg_input_info *)file;
        imb_tsk_info.data_src = DATA_SRC_SRAM;
        imb_tsk_info.cur_in_flash = 0;
    } else {
        imb_tsk_info.data_src = DATA_SRC_SRAM;
    ...
    if (__jpg_info) {
        ...
        imb_tsk_info.x_offset       = x;
        imb_tsk_info.y_offset       = y;
        imb_tsk_info.src_w          = __jpg_info->w;
        imb_tsk_info.src_h          = __jpg_info->h;
        imb_tsk_info.des_w          = width;
        imb_tsk_info.des_h          = height;
        imb_tsk_info.zip_en         = 0;
        ...
        imb_tsk_info.priv           = 0;
        imb_tsk_info.jpg_ginfo_cb   = __jpg_info->jpg_ginfo_cb;
        /* imb_tsk_info.jpg_fat_info  = __jpg_info->jpg_fat_info; */
        imb_tsk_info.jpg_fat_info  = __jpg_info;//必须确保指针指向的是全局的变量地址
    } else {
        imb_tsk_info.in_format      = file->format;
        imb_tsk_info.x_offset       = x;
        imb_tsk_info.y_offset       = y;
...
void imb_jpeg_test_sdcard()
{
    ...
    /* char *str_tab[] = {"storage/sd0/C/test6.jpg"}; */
    /* char *str_tab[] = {"storage/sd0/C/SD.jpg"}; */
...
static void ui_custom_draw_cb(int id, u8 *dst_buf, struct rect *dst_r, struct rect *src_r, u8 bytes_per_pixel, void *priv)
{
    ...
    if (id == DRAW_IMAGE) {
        ...
        /* int dst_dtride = (dst_r->width * bytes_per_pixel + 3) / 4 * 4; */
...
void imb_image_test(struct ui_res_info *info)
{
    ...
    ret = open_image_by_id(0, info->ui_resfile, &file, id & 0xff, id >> 16);
    ASSERT(!ret);
    imb_create_image_task(imb_task_get_id(), &file, info->ui_resfile, 50, 200, file.width, file.height, NULL, true, LOAD_FROM_FLASH_WITH_TAB);//width,height参数可小于file.width, file.height,若小于则裁剪
#endif
}
...
void imb_image_scale_test(struct ui_res_info *info)
{
    ...
    ret = open_image_by_id(0, info->ui_resfile, &file, id & 0xff, id >> 16);
    ASSERT(!ret);
    imb_create_image_scale_task(imb_task_get_id(), &file, info->ui_resfile, 1.5, 1.5, 180, 280, file.width * 1.5, file.height * 1.5, NULL, true, LOAD_FROM_FLASH_WITH_TAB);
    //4.缩放+裁剪测试
    id = PAGE0_5dad_05_MENU_ICON_BLOOD_SEL;
    ret = open_image_by_id(0, info->ui_resfile, &file, id & 0xff, id >> 16);
    ASSERT(!ret);
    ...
    float scale_ratio_w = 1.9;
    float scale_ratio_h = 1.9;
    imb_create_image_scale_task(imb_task_get_id(), &file, info->ui_resfile, scale_ratio_w, scale_ratio_h, scale_x, scale_y, file.width * scale_ratio_w, file.height * scale_ratio_h, image_set_crop(scale_x + file.width * 0.1f, scale_y + file.height * 0.1f, file.width * 1.7f, file.height * 1.7f), true, LOAD_FROM_FLASH_WITH_TAB);
#endif
}
...
void imb_image_rotate_chunk_test(struct ui_res_info *info)
{
    ...
    ret = open_image_by_id(0, info->ui_resfile, &file, id & 0xff, id >> 16);
    ASSERT(!ret);
    imb_create_image_rotate_chunk_task(imb_task_get_id(), &file, info->ui_resfile, 227, 227, 90, 227, 227);
#endif
}
...
void imb_image_rotate_test(struct ui_res_info *info)
{
    ...
    ret = open_image_by_id(0, info->ui_resfile, &file, id & 0xff, id >> 16);
    ASSERT(!ret);
    imb_create_image_rotate_task(imb_task_get_id(), &file, info->ui_resfile, 23, 133, 45, 227, 227, NULL, LOAD_FROM_FLASH_WITH_TAB);
#endif
}
...
void imb_string_test(struct ui_res_info *info)
{
    ...
    ret = open_string_by_id(0, info->ui_strfile, &file, id);
    ASSERT(!ret);
    imb_create_strpic_task(imb_task_get_id(), &file, info->ui_strfile, 150, 150, file.width, file.height, 0x001f);
#endif
}
...
static int gif_custom_read(void *fp, int offset, u8 *data, int len)
{
    /* printf("%s, fp : 0x%x, offset : 0x%x, data : 0x%x, len : 0x%x\n", __func__, fp, offset, data, len); */
    fseek(fp, offset, SEEK_SET);
    if (fread(fp, data, len) != len) {
        return -EFAULT;
    }
    return len;
}

void imb_gif_test(struct ui_res_info *info)
{
    int id;
    int ret;
    struct image_file file1;
    struct image_file file2;
    struct image_file file3;
    struct image_file tmpfile;
    u32 priv = UI_DATA_STORE_IN_NORFLASH ? &info->ui_resfile_info : 0;
    ...
    //创建背景任务(必须且需要在其他task之前创建)
    imb_create_bg_task(lcd_info.color_format, lcd_info.width, lcd_info.height, 0x0000);

    ////////////////////////////////////<地址映射表方式测试>////////////////////////////////////////

    id = PAGE0_466e_72; //低压缩
    ret = open_image_by_id(0, info->ui_resfile, &file1, id & 0xff, id >> 16);
    struct imb_task *gif_task1 = imb_create_gif_task(imb_task_get_id(), info->ui_resfile, &file1, priv, NULL, 50, 50, file1.width, file1.height);

    id = PAGE0_0d0a_112; //中压缩
    ret = open_image_by_id(0, info->ui_resfile, &file2, id & 0xff, id >> 16);
    ASSERT(!ret);
    struct imb_task *gif_task2 = imb_create_gif_task(imb_task_get_id(), info->ui_resfile, &file2, priv, NULL, 300, 100, file2.width, file2.height);
    ASSERT(!ret);
    id = PAGE0_d68f_TEST2; //高压缩
    ret = open_image_by_id(0, info->ui_resfile, &file3, id & 0xff, id >> 16);
    ASSERT(!ret);
    struct imb_task *gif_task3 = imb_create_gif_task(imb_task_get_id(), info->ui_resfile, &file3, priv, NULL, 100, 250, file3.width, file3.height);

    ////////////////////////////////////<绝对地址方式测试(只适用于norflash)>////////////////////////////////////////
    if (UI_DATA_STORE_IN_NORFLASH) {
        u32 gif_flash_addr = get_file_addr(RES_PATH"gif/giftest2.bin"); //获取norflash地址

        tmpfile.width = file3.width;
        tmpfile.height = file3.height;
        tmpfile.format = file3.format;
        tmpfile.offset = gif_flash_addr;//数据存储在norflash的地址
        tmpfile.len = file3.len;   //数据长度
        struct imb_task *gif_task4 = imb_create_gif_task(imb_task_get_id(), NULL, &tmpfile, NULL, NULL,  200, 150, tmpfile.width, tmpfile.height);
    }

    //////////////////////////////<FILE文件方式>///////////////////////////
    FILE *fp = fopen(RES_PATH"gif/giftest2.bin", "r");
    tmpfile.width = file3.width;
    tmpfile.height = file3.height;
    tmpfile.format = file3.format;
    tmpfile.offset = 0; //相对文件的偏移
    tmpfile.len = file3.len;
    struct imb_task *gif_task5 = imb_create_gif_task(imb_task_get_id(), fp, &tmpfile, NULL, gif_custom_read, 200, 150, tmpfile.width, tmpfile.height);

    imb_task_blend();

    /* imb_task_delete(imb_task_head_get(0), gif_task1); */
    /* imb_task_delete(imb_task_head_get(0), gif_task2); */
    // imb_task_delete(imb_task_head_get(0), gif_task3);
}
...
struct ui_res_info *ui_res_init()
{
    ...
    u8 magic[4] ALIGNED(4);
    res_fseek(ui_res_info_t.ui_resfile, 0, SEEK_SET);
    res_fread(ui_res_info_t.ui_resfile, magic, sizeof(magic));
    if ((magic[0] == 'R') && (magic[1] == 'U') && (magic[2] == '4') && (magic[3] == '1')) {
        ui_res_info_t.ui_resfile->ver = 0x0410;
    }
    ...
    ASSERT(ui_res_info_t.ui_strfile);
    res_fseek(ui_res_info_t.ui_strfile, 0, SEEK_SET);
    res_fread(ui_res_info_t.ui_strfile, magic, sizeof(magic));
    if ((magic[0] == 'R') && (magic[1] == 'U') && (magic[2] == '4') && (magic[3] == '1')) {
        ui_res_info_t.ui_strfile->ver = 0x0410;
    }
...
extern struct ui_msg msg_handle;
extern volatile u16 touch_msg_counter;
    ...
    while (1) {
        ...
        case UI_MSG_GIF_PLAY:
            ...
            if (!pagemove && (delay <= gif_delay)) {
                ...
                /* printf("gif_delay : %d, delay : %d, drop\n", gif_delay, delay); */
            }
            if (msg[6]) {
...
int imb_demo(void *arg)
{
    ...
    err = task_create(imb_task, arg, IMB_TASK_NAME);
    /* err = task_create(imb_api_task, arg, IMB_TASK_NAME); */
