...
@echo ********************************************************************************
@echo            SDK BR28 SAFE PACKAGE (KEEP TONE)
@echo ********************************************************************************
@echo %date% %time%
...
@rem Safety: keep tone.cfg in -res to avoid losing default tone files
@rem Safety: do not use any -format option in this script
..\..\isd_download.exe -tonorflash -dev br28 -boot 0x120000 -div8 -wait 300 -uboot uboot.boot -app app.bin cfg_tool.bin -res ui_upgrade p11_code.bin config.dat tone.cfg eq_cfg_hw.bin -flash-params flash_params.bin -uboot_compress -key %CHIPKEY%
...
copy /Y jl_isd.ufw update_safe_keep_tone.ufw
copy /Y update_safe_keep_tone.ufw "%OUT_DIR%\update_safe_keep_tone.ufw"

@rem ?????????????????????
copy /Y update_safe_keep_tone.ufw update.ufw

if exist db_update_data.bin (
    copy /Y db_update_data.bin update_safe_keep_tone.bin
    copy /Y update_safe_keep_tone.bin "%OUT_DIR%\update_safe_keep_tone.bin"
) else (
    @echo [WARN] db_update_data.bin not found, skip update_safe_keep_tone.bin
)
