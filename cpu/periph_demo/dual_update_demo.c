...
static u32 dual_ota_cache_size = 0;
static dual_ota_notify_cb_t dual_ota_notify_cb = NULL;
static void *dual_ota_notify_priv = NULL;
static u8 dual_ota_end_pending_verify = 0;
...
void dual_ota_set_notify_cb(dual_ota_notify_cb_t cb, void *priv)
{
    dual_ota_notify_cb = cb;
    dual_ota_notify_priv = priv;
}

void dual_ota_clear_notify_cb(void)
{
    dual_ota_notify_cb = NULL;
    dual_ota_notify_priv = NULL;
}
...
static int dual_ota_update_deal(u8 step, u8 *buf, u32 len);
...
static void dual_ota_fail_exit(void)
{
    ...
    dual_ota_cache_size = 0;
    dual_ota_end_pending_verify = 0;
    dual_ota_idle = 1;
...
static int dual_ota_write_cb(void *priv)
{
    ...
    if (ret) {
        ...
        if (dual_ota_end_pending_verify) {
            dual_ota_end_pending_verify = 0;
...
static int dual_ota_update_init(u32 file_size, u32 file_crc, u32 pkt_len)
{
    ...
    if (file_size == 0 || pkt_len == 0 || pkt_len > MAX_PACKET_LEN) {
        return -4;
    }
    if(bin_buf) return -1;
...
int dual_ota_app_data_deal(u32 msg, u8 *buf, u32 len)
{
    ...
    if ((dual_ota_step == DUAL_OTA_STEP_START) && (len == 10)
        && (buf[0] == 0x55) && (buf[1] == 0xAA)) {
        if (dual_ota_step != DUAL_OTA_STEP_START) {
            log_error("%s start re-entry", __func__);
            dual_ota_fail_exit();
            return -2;
        }
        u32 data[2];
    ...
    if (dual_ota_step != DUAL_OTA_STEP_WRITE) {
        log_error("%s receive data before start", __func__);
        dual_ota_fail_exit();
        return -3;
    }

    if ((len == 2) && (buf[0] == 0xAA) && (buf[1] == 0x55)) {
        if ((u32)(dual_ota_written_size + dual_ota_cache_size) != ota_info.file_size) {
            log_error("%s end size mismatch, written:%d cache:%d total:%d expect:%d",
                      __func__, dual_ota_written_size, dual_ota_cache_size,
                      dual_ota_written_size + dual_ota_cache_size, ota_info.file_size);
            dual_ota_fail_exit();
            return -4;
        }
        if (dual_ota_cache_size > 0) {
            dual_ota_end_pending_verify = 1;
            ret = dual_ota_update_deal(DUAL_OTA_STEP_WRITE, bin_buf, dual_ota_cache_size);
            dual_ota_cache_size = 0;
            if (ret) {
                dual_ota_fail_exit();
            }
    ...
    if ((u32)(dual_ota_written_size + dual_ota_cache_size + len) > ota_info.file_size) {
        log_error("%s data size overflow, written:%d cache:%d recv:%d expect:%d",
                  __func__, dual_ota_written_size, dual_ota_cache_size, len, ota_info.file_size);
        dual_ota_fail_exit();
        return -5;
    }
    /*判断 */
    while (len > 0) {
        u32 free_len = ota_info.max_pkt_len - dual_ota_cache_size;
        u32 cp_len = (len < free_len) ? len : free_len;
        memcpy(bin_buf + dual_ota_cache_size, buf, cp_len);
        dual_ota_cache_size += cp_len;
        buf += cp_len;
        len -= cp_len;

        if (dual_ota_cache_size == ota_info.max_pkt_len) {
            ret = dual_ota_update_deal(DUAL_OTA_STEP_WRITE, bin_buf, dual_ota_cache_size);
            dual_ota_cache_size = 0;
            if (ret) {
                dual_ota_fail_exit();
...
{
    if (dual_ota_notify_cb) {
        return dual_ota_notify_cb(msg, buf, len, dual_ota_notify_priv);
    }

    extern bool uart1_send_toMCU(uint16_t protocol_id, uint8_t *data, uint16_t len);
    int ret = 0;
    u8 rsp = 0x02;
    (void)buf;
    (void)len;
    ...
    switch(msg)
    {
    case DUAL_OTA_NOTIFY_CONTINUE:
        rsp = 0x00;
        break;
    case DUAL_OTA_NOTIFY_SUCCESS:
        rsp = 0x01;
        break;
    case DUAL_OTA_NOTIFY_FAIL:
    default:
        rsp = 0x02;
        break;
    }
    if (!uart1_send_toMCU(0x55AA, &rsp, 1)) {
        ret = -1;
    }
    return ret;
}
