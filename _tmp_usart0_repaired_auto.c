// 注意�?uart_test.text 段在部分固件配置下不�?PC �?��行范围内�?
// 若把关键逻辑放入该�?会触�?CPU pc_limit 异常�?
// 因�?默�?不启�?��仅在�?���?要调试�?隔�?时显式打�? UART1_TEST_SECTION_ENABLE�?
...
// 说明：工程中部分�?��方头文件会用宏重新定�?bool（例如映射到 _Bool），
// �?cpu.h �?bool �?typedef(u8)；两者叠加会导致“头文件声明 vs 源文件定义�?�类型不�?致�?
// 这里在编译单元内取消 bool 宏，�?���?��件使�?cpu.h �?typedef bool�?
#ifdef bool
#undef bool
#endif
